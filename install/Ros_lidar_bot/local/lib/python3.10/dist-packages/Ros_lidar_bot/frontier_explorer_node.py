...
class FrontierExplorer(Node):
    def __init__(self) -> None:
       ...
       # self.declare_parameter("no_frontier_done_ticks", 4)
        ...
        self.declare_parameter("home_recovery_ticks", 6)
        self.declare_parameter("no_frontier_done_ticks", 25)
        ...
        self.home_recovery_ticks = int(p("home_recovery_ticks").value)
    ...
    def _tick(self) -> None:
        ...
        if self._home_xy is None:
            ...
            self.get_logger().info(
                f"Home position set: {self._home_xy[0]:.2f}, {self._home_xy[1]:.2f}"
            )    
        ...
        if target is None:
            ...
            if (self._no_frontier_ticks >= self.home_recovery_ticks
                    and self._home_xy is not None
                    and not self._returning_home):
                self.get_logger().info(
                    f"No frontier for {self._no_frontier_ticks} ticks — returning home"
                )
    ...
    def _send_home_goal(self) -> None:
        ...
        pose = self._build_goal_pose(
            self._home_xy[0], self._home_xy[1], rx, ry
        )
        self._returning_home = True
        self._send_goal(pose, score=-999.0)  
        if self._returning_home and not self.goal_in_progress:
            # home goal failed to even send or was rejected
            # just reset and try frontier search anyway
            self._returning_home = False
            self._no_frontier_ticks = 0 
    ...
    def _goal_result_cb(self, future) -> None:
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if self._returning_home:
                self.get_logger().info("Returned home — resuming frontier search")
                self._returning_home = False
                self._no_frontier_ticks = 0  
            self.active_goal_xy = None
            self.active_goal_score = -float("inf")
